...
typedef struct {
	double  const  (*pdModip)[39][39];       /*// grid of modified dip latitude values*/
} MODIP_st;

typedef struct {
	double  const (*pdF2)[12][2][76][13];   /*// CCIR coeffs for computing FoF2*/
	double  const  (*pdM3000)[12][2][49][9]; /*// CCIR coeffs for computing M(3000)F2*/
} CCIR_st;
